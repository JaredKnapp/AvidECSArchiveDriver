...
// Avid cross-platform fix by C.Erickson
//
// This allows us to use the same source tree for both Windows and Mac
// This header was configured by using the following command in a clean source tree:
// ./configure --disable-shared --disable-dependency-tracking --enable-netaccessor-cfurl --enable-transcoder-macosunicodeconverter --enable-msgloader-inmemory CFLAGS="-Os -arch i386 -arch x86_64" CXXFLAGS="-Os -arch i386 -arch x86_64"
// And it corresponds to the 32/64 universal Mac build.
#if defined(_MSC_VER)
// switch over to the vanilla MSVC header
#include "Xerces_autoconf_config.msvc.hpp"
#else
// from here until the #endif at the end is the standard Xerces 3.0.1 header configured for Mac OS X
...
#endif // ! defined(_MSC_VER) // Avid cross-platform fix by C.Erickson
